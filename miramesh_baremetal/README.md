...
### How to build
To be able to build this example one need to have downloaded the nrf5-sdk from Nordic Semiconductors. It is recommended to put nrf5-sdk in the same folder as mira-examples and it is required to rename the folder to nrf5-sdk. It is also possible specify the location of nrf5-sdk when building by providing `SDKDIR` when running make.

To build the example, in this directory run:
```
make all
```
The example assumes that libmira and nrf5-sdk is placed in the same folder as mira-examples, to specify another path run:
```
